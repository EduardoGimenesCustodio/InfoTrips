function PaisDAO(connection){
	...
	PaisDAO.prototype.buscarPaisComFavoritos = function(pais_busca, email, callback){
		this._connection.query('select nome_pais, bandeira_pais, isfavorite_favorito, id_favorito from pais inner join favorito on id_pais = pais_favorito where usuario_favorito = (select id_usuario from usuario where email_usuario = "'+ email +'") AND nome_pais like "%' + pais_busca + '%" AND nome_pais != "Brasil"', callback);
	}
